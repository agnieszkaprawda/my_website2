...
We were asked to carry out the analysis. The objective is to find out whether there is indeed a significant difference between the salaries of men and women, and whether the difference is due to discrimination or whether it is based on another, possibly valid, determining factor. 
...
We will perform different types of analyses, and check whether they all lead to the same conclusion 
...
First, we will calculate summary statistics on salary by gender. Also, we will create and print a dataframe where, for each gender, we show the mean, SD, sample size, the t-critical, the SE, the margin of error, and the low/high endpoints of a 95% condifence interval.
